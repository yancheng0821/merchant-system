package com.merchant.server.notificationservice.aspect;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.merchant.server.notificationservice.client.AuditClient;
    ...
    public Object auditOperation(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            if (auditable.recordOldValue()) {
                // 对于CREATE/PURCHASE/UPDATE操作，直接序列化返回结果
    ...
    private Object getOldValue(String resource, Long resourceId, Long tenantId) {
        ...
        return readOnlyTransactionTemplate.execute(status -> {
            try {
                ...
                if ("NOTIFICATION_TEMPLATE".equals(resource)) {
                    // 获取TemplateService
                    Object templateService = applicationContext.getBean("templateServiceImpl");
                    Method getById = templateService.getClass().getMethod("getTemplateById", Long.class);
                    return getById.invoke(templateService, resourceId);
                } else if ("NOTIFICATION_LOG".equals(resource)) {
                    // 获取NotificationService
                    Object notificationService = applicationContext.getBean("notificationServiceImpl");
                    Method getById = notificationService.getClass().getMethod("getNotificationLogById", Long.class);
                    return getById.invoke(notificationService, resourceId);
                }
                // 可以添加其他资源类型的处理
            } catch (Exception e) {
    ...
    private Map<String, Object> extractCoreFields(Object obj) {
        ...
        try {
            ...
            String[] importantFields = {
                ...
                // 通知模板相关
                "templateCode", "templateName", "subject", "content",
                // 通知日志相关
                "recipient", "businessId", "businessType", "sentAt", "retryCount", "errorMessage"
            };
