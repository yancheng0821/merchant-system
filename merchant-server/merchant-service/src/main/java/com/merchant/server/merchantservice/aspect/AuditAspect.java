package com.merchant.server.merchantservice.aspect;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.merchant.server.merchantservice.client.AuditClient;
...
public class AuditAspect {
    ...
    public Object auditOperation(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (auditable.recordOldValue() && resourceId != null) {
            ...
            oldValue = getOldValue(auditable.resource(), resourceId, tenantId, args, parameters);
            if (oldValue != null) {
        ...
        try {
            ...
            if (auditable.recordOldValue()) {
                // 对于CREATE/PURCHASE/UPDATE操作，重新查询获取新值
                if ("CREATE".equals(auditable.action()) ||
                    "PURCHASE".equals(auditable.action()) ||
                    "UPDATE".equals(auditable.action())) {
                    Object newValue = fetchNewValue(auditable.resource(), resourceId, tenantId, result, args, parameters);
                    if (newValue != null) {
                        newValueJson = serializeToJson(newValue);
                        log.debug("New value captured: {}", newValueJson);
                    } else {
                        log.warn("New value is null after fetching");
                    }
                }
    ...
    protected void recordAuditAsync(Long tenantId, Long userId, String resource, String action,
        ...
        try {
            // 如果是UPDATE操作，且新旧值相同，则跳过审计日志
            if ("UPDATE".equals(action) && oldValue != null && newValue != null) {
                if (oldValue.equals(newValue)) {
                    log.debug("新旧值相同，跳过审计日志 - resource: {}, resourceId: {}", resource, resourceId);
                    return;
                }
            }
    ...
    private String extractConfigKey(Object[] args, Parameter[] parameters) {
        for (int i = 0; i < parameters.length; i++) {
            Parameter param = parameters[i];
            PathVariable pathVariable = param.getAnnotation(PathVariable.class);
            if (pathVariable != null) {
                String pathVarName = pathVariable.value().isEmpty() ? param.getName() : pathVariable.value();
                if ("configKey".equals(pathVarName) && args[i] instanceof String) {
     ...
     */
    private Object getOldValue(String resource, Long resourceId, Long tenantId, Object[] args, Parameter[] parameters) {
        // 在独立的只读事务中执行查询，避免读到当前事务中已修改的数据
        return readOnlyTransactionTemplate.execute(status -> {
            try {
                // 对于MERCHANT_CONFIG_ITEM，需要提取configKey参数
                if ("MERCHANT_CONFIG_ITEM".equals(resource)) {
                    String configKey = extractConfigKey(args, parameters);
                    if (configKey != null && tenantId != null) {
                        Object merchantConfigService = applicationContext.getBean("merchantConfigServiceImpl");
                        Method getConfigByKey = merchantConfigService.getClass().getMethod("getConfigByKey", Long.class, String.class);
                        return getConfigByKey.invoke(merchantConfigService, tenantId, configKey);
                    }
                    return null;
                }
                ...
                } else if ("MERCHANT".equals(resource)) {
                    // 获取MerchantService
                    Object merchantService = applicationContext.getBean("merchantServiceImpl");
                    Method getByTenantId = merchantService.getClass().getMethod("getMerchantByTenantId", Long.class);
                    return getByTenantId.invoke(merchantService, tenantId);
                } else if ("MERCHANT_CONFIG".equals(resource)) {
                    // 获取MerchantConfigService - 获取完整配置
                    Object merchantConfigService = applicationContext.getBean("merchantConfigServiceImpl");
                    Method getMerchantConfig = merchantConfigService.getClass().getMethod("getMerchantConfig", Long.class);
                    return getMerchantConfig.invoke(merchantConfigService, tenantId);
                }
                // 可以添加其他资源类型的处理
            } catch (Exception e) {
    ...
    private Map<String, Object> extractCoreFieldsForAudit(Object obj) {
        ...
        try {
            ...
            // 特殊处理 MerchantConfigDTO (商户配置)
            if ("MerchantConfigDTO".equals(className) || className.contains("MerchantConfig")) {
                return extractMerchantConfigFields(obj);
            }
     ...
     * 提取商户配置的核心字段
     */
    private Map<String, Object> extractMerchantConfigFields(Object obj) {
        Map<String, Object> result = new HashMap<>();
        try {
            // 使用ObjectMapper直接序列化，保留完整的嵌套结构
            return objectMapper.convertValue(obj, new com.fasterxml.jackson.core.type.TypeReference<Map<String, Object>>() {});
        } catch (Exception e) {
            log.warn("Failed to extract MerchantConfig fields", e);
    ...
    private Map<String, Object> extractCoreFields(Object obj) {
        ...
        try {
            ...
            String[] importantFields = {
                ...
                // 商户相关
                "tenantId", "merchantCode", "merchantName", "merchantType",
                "parentMerchantId", "businessCategory", "businessLicense",
                "legalPerson", "contactPerson", "contactPhone", "contactEmail",
                "province", "city", "postCode", "longitude", "latitude", "timezone"
            };
     ...
     * 在操作完成后获取新值
     */
    private Object fetchNewValue(String resource, Long resourceId, Long tenantId, Object returnValue, Object[] args, Parameter[] parameters) {
        try {
            if (resourceId == null && tenantId == null) {
                return extractBodyFromResult(returnValue);
            }

            if ("MERCHANT".equals(resource) && tenantId != null) {
                Object merchantService = applicationContext.getBean("merchantServiceImpl");
                Method getByTenantId = merchantService.getClass().getMethod("getMerchantByTenantId", Long.class);
                return getByTenantId.invoke(merchantService, tenantId);
            } else if ("MERCHANT_CONFIG".equals(resource) && tenantId != null) {
                Object merchantConfigService = applicationContext.getBean("merchantConfigServiceImpl");
                Method getMerchantConfig = merchantConfigService.getClass().getMethod("getMerchantConfig", Long.class);
                return getMerchantConfig.invoke(merchantConfigService, tenantId);
            } else if ("MERCHANT_CONFIG_ITEM".equals(resource) && tenantId != null) {
                String configKey = extractConfigKey(args, parameters);
                if (configKey != null) {
                    Object merchantConfigService = applicationContext.getBean("merchantConfigServiceImpl");
                    Method getConfigByKey = merchantConfigService.getClass().getMethod("getConfigByKey", Long.class, String.class);
            ...
            log.warn("Failed to fetch new value for resource: {}, tenantId: {}, error: {}", resource, tenantId, e.getMessage());
        }
